...
void InterfejsUzytkownika::menuWykonajExport() {
    int coEksportowac;
    cout << "Co chcesz wyeksportowac? (1 - cala liste, 2 - wybrana osobe): ";
    cin >> coEksportowac;

    IEksportowalny* cel = nullptr;

    if (coEksportowac == 1) {
        int nr;
        cout << "Ktora liste wyeksportowac (1-" << maxList << "): ";
        cin >> nr;
        if (nr >= 1 && nr <= maxList) {
            cel = &tablicaList[nr - 1];
        } else {
            cout << "Zly numer listy\n";
            return;
        }
    } else if (coEksportowac == 2) {
        string nazwisko;
    ...
    int format;
    cout << "Wybierz format eksportu (1 - CSV, 2 - TXT): ";
    cin >> format;

    string nazwaPliku;
    cout << "Podaj nazwe pliku (np. wynik.csv lub wynik.txt): ";
    cin >> nazwaPliku;

    IExporter* eksporter = nullptr;
    if (format == 1) {
        eksporter = new ExporterCSV();
    } else if (format == 2) {
        eksporter = new ExporterTXT();
    } else {
        cout << "Nieznany format\n";
        return;
    }

    eksporter->wykonajExport(cel, nazwaPliku);
    delete eksporter;
}
...
void InterfejsUzytkownika::petla() {
    ...
    do {
        ...
        cout << "5. edytuj dane\n";
        cout << "6. EKSPORTUJ DANE (IExporter - CSV/TXT)\n";
        cout << "q. wyjscie\n";
        cout << "wybor: ";
        cin >> wybor;
        ...
        switch (wybor) {
            ...
            case '6': menuWykonajExport(); break;
        }
    } while (wybor != 'q');
}